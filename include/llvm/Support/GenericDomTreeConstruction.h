...
struct SemiNCAInfo {
    ...
    static ResultTy Get(NodePtr N, BatchUpdatePtr BUI) {
      ...
      for (auto ChildAndKind : FCIt->second) {
        ...
        if (UK == UpdateKind::Insert) {
          ...
          auto RIt = llvm::find(Res, Child);
          assert(RIt != Res.end() && "Expected child not found in the CFG");
          DEBUG(dbgs() << "\tHiding edge " << BlockNamePrinter(N) << " -> "
                       << BlockNamePrinter(Child) << "\n");
          std::swap(*RIt, Res.back());
          Res.pop_back();
        } else {
  ...
  static TreeNodePtr AddNewTrivialRoot(DomTreeT& DT, const BatchUpdatePtr BUI,
  ...
  static void InsertEdge(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    if (!FromTN) {
      ...
      if (HasForwardSuccessors(From, BUI)) {
      ...
      FromTN = AddNewTrivialRoot(DT, BUI, From);
  ...
  static void DeleteEdge(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    if (!FromTN){
      if (IsPostDom && !HasForwardSuccessors(To, BUI)) {
  ...
  static void DeleteUnreachable(DomTreeT &DT, const BatchUpdatePtr BUI,
    ...
    if (IsPostDom && !HasForwardSuccessors(To, BUI)) {
      CalculateFromScratch(DT, BUI);  // Deletion makes To reverse-reachable.
      return;
    }
