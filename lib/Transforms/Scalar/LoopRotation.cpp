...
bool LoopRotate::rotateLoop(Loop *L, bool SimplifiedLatch) {
  ...
  if (DT) {
    // The OrigPreheader branches to the NewHeader and Exit now. Perform the
    // insertions first not to disconnect and immediately reconnect any subtree.
    // Then, inform the DT about the removed edge to the OrigHeader (that got
    // removed).
    DT->insertEdge(OrigPreheader, Exit);
    DT->insertEdge(OrigPreheader, NewHeader);
    DT->deleteEdge(OrigPreheader, OrigHeader);
  }
